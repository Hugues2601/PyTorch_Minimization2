from DataProcessing import data_processing, api_retriever, get_treasury_yield
...
def run():
    df, market_prices, T, IV, K = data_processing(CONFIG.symbol)
    print(f"Nb of options :{len(market_prices)} | mean price: {np.mean(market_prices)}")
    initial_guess = CONFIG.initial_guess
    calibrated_params = calibrate(CONFIG.S0, market_prices, K, T, CONFIG.r, initial_guess, max_epochs=5000, loss_threshold=1)
